...
<script type="text/javascript">
	...
	function loadCSS() {
		...
		sheet.setAttribute("href", "{/literal}{$pqpCss}{literal}");
				...
				{foreach from=$logs.console item=log}
					<tr class='log-{$log.type}'>
						...
						<td class="{cycle values="alt,"}">
							{if $log.type == 'log'} 
								<div><pre style="background: transparent;">{$log.data}</pre></div>
							{elseif $log.type == 'memory'}
								<div><pre style="background: transparent;">{$log.data}</pre> <em>{$log.dataType}</em>: {$log.name} </div>
							{elseif $log.type == 'speed'}
								<div><pre style="background: transparent;">{$log.data}</pre> <em>{$log.name}</em></div>
