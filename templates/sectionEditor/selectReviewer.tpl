...
<p><a class="action" href="{url op="enrollSearch" path=$submission->getSectionId()}">{translate key="sectionEditor.review.enrollReviewer"}</a>&nbsp;|&nbsp;<a class="action" href="{url op="createReviewer" path=$submission->getSectionId()}">{translate key="sectionEditor.review.createReviewer"}</a>&nbsp;|&nbsp;<a class="action" href="{url op="createExternalReviewer" path=$submission->getSectionId()}">{translate key="sectionEditor.review.createExternalReviewer"}</a>{foreach from=$reviewerDatabaseLinks item="link"}{if !empty($link.title) && !empty($link.url)}&nbsp;|&nbsp;<a href="{$link.url|escape}" target="_new" class="action">{$link.title|escape}</a>{/if}{/foreach}</p>
